...
The **evidence under evaluation** is the [simplification proposal](../../docs/gen2/BEAUTYQ_SEARCH_GEN2_SIMPLIFICATION_PLAN.md) (status *proposed*; explicitly not an approved implementation plan). This feature treats the proposal as **attributed criticism plus a proposed alternative** — nothing more. It is NOT proof that:
...
**This specification produces a decision contract only.** No candidate is evaluated here; the evaluation is the execution phase. No implementation of any disposition happens under this feature under any outcome, including full approval of every candidate (see FR-009).
...
There is no closeout, promotion, independent-verify, or other delivery precondition. This decision contract may exist and be reviewed on its own, and substantive execution may begin whenever the human chooses to invoke it. The decision evaluates the current source/architecture state it explicitly records as its evaluated state; if that state moves **before a human verdict**, affected evidence is re-attributed per Principle V — findings bound to the earlier state stay attributed to it and are marked superseded, never silently reattributed. **After a human verdict**, the recorded verdict remains historical evidence bound to the state actually decided and is not rewritten or reopened by later drift; using a recommendation against a materially changed state requires a new evaluation against that state.
...
| --- | --- | --- |
| Simplification proposal | Attributed criticism and a proposed alternative; its unresolved candidate list seeds the candidate inventory | Non-normative. Never an approval, never current architecture. Its diagnoses stay attributed to it (Principle VIII). |
| Operations runbook / technical specification | Current obligations the decision evaluates candidates against | Normative current owners. The decision evaluates, never rewrites them (any owner update belongs to a separately authorized implementation task outside this decision contract). |
...
The decision phase MAY extend the candidate inventory beyond the proposal's list when the current source state or audit findings surface additional candidates, and MAY admit, split, merge, or reject proposal milestones as provisional decision units. It is not bound to the proposal's milestone numbering; it MUST track which proposal milestone (if any) each candidate came from for traceability. A `REJECTED` provisional unit produces no final Candidate and receives no disposition (FR-004).
...
| The deferred-but-unapproved simplification ideas under evaluation | the simplification proposal (non-normative here) |
...
**Why this priority**: The proposal bundles structurally different questions (trace owners, project consolidation, remaining test consolidation, and Qdrant value) whose evidence bases differ. Accepting or rejecting the bundle would hide which specific mechanisms are justified. Per-candidate assessment is the core decision product.

**Independent Test**: For each final Candidate in the decision record, an auditor can find all twelve assessment dimensions completed, exactly one disposition, a stated confidence/uncertainty level, and a named human-approval requirement — and can follow the evidence references to the evaluated source/architecture state and the audit finding cited where one exists; a provisional unit rejected as a decision unit instead shows its traceability and basis and receives no disposition.

**Acceptance Scenarios**:

1. **Given** a final Candidate, **When** it is assessed, **Then** the record covers, at minimum: the current obligation the mechanism serves; whether that obligation still exists at the evaluated source state; the demonstrated maintenance/complexity cost; the evidence of value/risk; the relevant Distage/Izumi audit classification for the mechanism (where available); what breaks or becomes unowned if the mechanism is removed; whether documentation-only consolidation is sufficient; whether framework reuse (existing primitive) is preferable; whether BeautyQ-local retention is preferable; a recommendation; explicit confidence/uncertainty; and the human approval requirement.
...
3. **Given** the human verdict exists (approve, reject, defer, or narrow — per candidate or for the whole package), **When** the record is finalized, **Then** the verdict is recorded verbatim as the decision outcome; an approved disposition may inform a later separately scoped implementation task, but any such task requires separate explicit human authorization and is not created or authorized by this feature; a deferral records the deferred state without starting any work.
...
- A disposition is approved by the human but no implementation task has yet been separately authorized: that is a valid CLOSED outcome of this feature — the recorded verdict may inform a separately scoped, explicitly human-authorized task if and when the human chooses; nothing is implemented by or under this feature.
...
- **FR-004** The candidate inventory distinguishes a **provisional candidate/inventory unit** from a **final material Candidate**. A provisional unit MAY be `ADMITTED`, `SPLIT`, `MERGED`, or `REJECTED`; it carries traceability to its originating proposal milestone (where one exists) and a recorded basis. A `REJECTED` provisional unit produces no final Candidate and receives **no** disposition (`REJECTED` is not a disposition). Every final Candidate MUST receive all twelve assessment dimensions (US-1 scenario 1) and exactly one disposition from FR-005. `INSUFFICIENT_EVIDENCE` remains the disposition for a real final Candidate whose decision lacks necessary evidence; it is not a provisional-unit rejection label.
...
- **FR-007** Every disposition MUST cite the evaluated source/architecture state and, where available, the audit finding it uses; dispositions MUST NOT float to a state other than the one evaluated (Principle V).
...
- **FR-009** This decision contract authorizes no implementation. A human-approved disposition may inform a separately scoped implementation task, but source/build/runtime changes require explicit human authorization for that task. The human may use Spec Kit for that implementation boundary, but this decision feature neither creates nor requires a particular follow-up mechanism. Where such a task is separately authorized, it carries the exact accepted candidates, retained obligations, migration requirements, validation requirements, and rollback/operational implications where applicable.
...
- **Provisional candidate/inventory unit**: a candidate-derivation entry that may be `ADMITTED`, `SPLIT`, `MERGED`, or `REJECTED`; carries proposal traceability (where one exists) and a recorded basis. A `REJECTED` unit produces no final Candidate and receives no disposition.
- **Candidate (final decision unit)**: one simplifiable mechanism or coherent mechanism group that survives provisional derivation; sourced from the proposal (tracked to its milestone) or surfaced by the evaluated source state/audit. Receives all twelve assessment dimensions and exactly one disposition.
- **Proposal**: the simplification proposal — attributed criticism and a proposed alternative; non-normative input, never authority.
- **Evaluated source/architecture state**: the exact source/architecture state the decision records as its attribution anchor (Principle V).
...
- **Disposition**: one of the six fixed states per final Candidate (`KEEP`/`SIMPLIFY_LOCALLY`/`REPLACE_WITH_EXISTING_FRAMEWORK`/`FRAMEWORK_FOLLOWUP_REQUIRED`/`REMOVE`/`INSUFFICIENT_EVIDENCE`), with confidence/uncertainty — as produced by the agent/coordinator, always a **recommendation** until the human verdict exists; the verdict, not the recommendation, is the decision outcome.
...
- **Source-state drift before a human verdict** → affected evidence marked superseded and affected candidates return to evaluation; dispositions are never silently reattributed (Principle V). Before recommendations are used against a materially changed state, a new evaluation must be performed against that state.
- **Source-state drift after a human verdict (CLOSED)** → the recorded human verdict remains immutable historical evidence bound to the state actually decided; later source drift does not rewrite or reopen it. Using a recommendation against a materially changed state requires a new evaluation against that state. This requires no version ledger, digest, or synthetic state identity.
- **Approved disposition without a follow-up authorization** → valid closed outcome; the recorded verdict may inform a separately scoped, explicitly human-authorized task if and when the human chooses, and stops (FR-009).
...
- implement generic framework functionality (framework extraction belongs to a separately authorized framework follow-up where `FRAMEWORK_FOLLOWUP_REQUIRED` is approved);
...
In scope: authoring this decision contract; the decision phase that assembles the candidate inventory from the proposal plus current-source-state/audit evidence, assesses each final Candidate per FR-004, assigns dispositions per FR-005, and produces the synthesis and named human-approval requirements per FR-013/FR-010.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: 100% of decision-phase actions record the source/architecture state evaluated; 0 candidates are assessed against an unrecorded state.
- **SC-002**: 100% of final Candidates carry all twelve assessment dimensions and exactly one disposition; a rejected provisional unit carries its traceability and basis and no disposition; 0 candidates are decided as part of an unexamined bundle.
- **SC-003**: 0 proposal diagnoses are restated as established project fact anywhere in the decision record; every cost/overengineering/deletion-benefit claim traces to evidence from the evaluated source/architecture state or is attributed to the proposal as its claim (auditable by reference check).
- **SC-004**: 0 change-oriented dispositions lack: explicit retained obligations, an explicit break/unowned-on-removal analysis, and a named human approval requirement; 100% require explicit human authorization for any separately scoped implementation task and follow from no source change under this feature.
...
- **SC-007**: 100% of dispositions cite the evaluated source/architecture state and, where applicable, the audit finding; 0 dispositions float onto an unevaluated state (Principle V).
...
- The proposal under evaluation is the current [simplification proposal](../../docs/gen2/BEAUTYQ_SEARCH_GEN2_SIMPLIFICATION_PLAN.md); candidates already resolved by an authorized cleanup are out of scope for this decision.
...
- The candidate inventory at execution will be derived from the proposal's still-unresolved candidates plus any additional candidates the current source state and audit findings surface; the exact candidate list is a decision-phase product, not fixed here.
