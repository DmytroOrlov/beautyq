...
**Status**: Draft — durable **research contract**. Specification only; research **execution is gated** (see Execution Gate). This document authorizes no source, framework, or architecture change.
...
**Input**: User description (abridged): "A read-only research contract that determines whether infrastructure/mechanics BeautyQ implemented locally are already well served by existing Distage/Izumi primitives and correctly used, supported but underused, supported but hard to discover, genuinely missing as reusable framework primitives, or inherently BeautyQ/domain-specific and correct to keep local. Optimize for discovering BOTH framework opportunities and evidence that no framework extraction is warranted; it is not an extraction mandate. Execution is gated on completion of the current accepted BeautyQ delivery closeout. Implementation/refactoring derived from the audit is not authorized."

## Overview and Research Object

This feature is the read-only **Distage/Izumi leverage and framework-opportunity audit** that the Q2 delivery-closeout feature explicitly deferred as its single follow-up (`specs/002-beautyq-q2-closeout/spec.md`, §"Deferred Follow-up"). Its object of study is the boundary between *reusable Distage/Izumi framework mechanics* and *BeautyQ-owned domain mechanics*, as that boundary is actually realized in the accepted BeautyQ Search Gen2 baseline.
...
This specification produces a **research contract only**. The mechanical mapping, source comparison, and classification work happen in the gated execution phase, not here.

## Execution Gate (mandatory, blocking)

**Research execution begins only after the current accepted BeautyQ delivery closeout reaches its required verified closeout boundary** — the verified closeout boundary owned by feature `specs/002-beautyq-q2-closeout`, whose protected workflows are procedured by the [operations runbook](../../docs/gen2/BEAUTYQ_SEARCH_GEN2_OPERATIONS.md). The [post-cutover plan](../../docs/gen2/BEAUTYQ_SEARCH_GEN2_POST_CUTOVER_PLAN.md) documents the historical origin of the accepted boundary set and is source/history context only; it is not the gate authority.

*Reason*: the audit must study a **stable, accepted BeautyQ baseline**, not a moving source/evidence target while acceptance is still open. Classifying a local mechanic against a framework primitive is meaningless if the mechanic itself is still mid-acceptance.

Gate rules for this contract:

- The specification **may exist and be reviewed before** the gate closes; authoring this contract is not blocked by it.
- **No** research-phase activity that reads BeautyQ source/tests, enumerates framework primitives against those needs, or produces classifications may be performed until the gate is independently recorded as satisfied.
- Opening the gate is evidenced, not asserted: execution requires a durable reference to the accepted-closeout state (the verified boundary's own evidence) plus the exact BeautyQ baseline (source identity) the audit will study. Absent that, the correct output is `AUDIT — GATE NOT YET SATISFIED`, naming the unfinished closeout boundary and its owner (Constitution Principles III, V).
...
The later post-closeout **simplification decision** is now a materialized contract — [`specs/004-beautyq-post-closeout-simplification-decision`](../004-beautyq-post-closeout-simplification-decision/spec.md) — and that contract names **completion of this audit as one of its own execution dependencies** (`DEP-FRAMEWORK-AUDIT`). The relationship is therefore an **asymmetric dependency**, stated precisely so it cannot be read two ways:

- **003 completes on its own synthesis.** Its completion condition is internal — its synthesis answering its standing questions on their own terms (Constitution Principle III). It can be COMPLETE whether or not 004 ever runs.
- **003 does not itself authorize simplification.** Its findings must be usable by the decision **without authorizing it** (Constitution Principles IV, VIII).
- **003 does not perform the decision.** Candidate evaluation, dispositions, and the human decision belong wholly to 004; running 004 is out of scope here (Non-Goals).
- **But 003 completion is an explicit INPUT/DEPENDENCY gate for substantive execution of 004.** 004's `DEP-FRAMEWORK-AUDIT` refuses to allow candidate evaluation until this audit's completed synthesis is recorded. In one line: **003 can be COMPLETE without 004 running, but 004 cannot substantively execute before 003 is COMPLETE.**
- **The dependency runs downstream and does not fold back into 003.** 004 is not part of this audit's completion gate; whether, when, or how 004 runs never changes whether 003 is finished.

The earlier framing that this audit is "upstream evidence … not a gate on" the decision is **retired as inaccurate**: now that 004 is materialized, this audit is a *required input* to that decision's execution while still granting it no permission.
...
| Current delivery/remaining-work status and the closeout this audit waits on | the Q2 closeout feature (`specs/002-beautyq-q2-closeout`) |
...
1. **Given** the gated baseline and a mapped mechanic, **When** it is classified, **Then** it receives exactly one of `WELL_USED` / `UNDERUSED` / `HARD_TO_DISCOVER` / `MISSING_GENERIC_PRIMITIVE` / `BEAUTYQ_SPECIFIC`, with the framework-version and BeautyQ-baseline references that support the label.
...
- The gate is not satisfied when execution is attempted: produce `AUDIT — GATE NOT YET SATISFIED`, name the unfinished closeout boundary and owner; do not begin mapping.
...
- The accepted baseline moved after the gate opened: re-anchor the BeautyQ baseline identity; findings bound to the old baseline stay attributed to it and are marked superseded, not silently reattributed (Constitution Principle V).
...
- **FR-002** Research **execution** MUST NOT begin until the accepted BeautyQ delivery closeout reaches its required verified closeout boundary, evidenced by a durable reference to that verified state and to the exact BeautyQ baseline to be studied (see Execution Gate). Until then, the only valid output is a gate report.
...
- **FR-009** Research execution MUST combine all six method stages, in this dependency order, with later stages consuming earlier artifacts:
  1. **A — Exhaustive/mechanical BeautyQ usage mapping**: enumerate the local infrastructure/mechanics sites from the accepted baseline without yet judging them.
...
- **FR-017** This feature MUST NOT perform, approve, or reopen post-closeout **simplification**. Completion of this audit is a **required INPUT/DEPENDENCY gate** for substantive execution of the materialized decision feature `004-beautyq-post-closeout-simplification-decision`, via that feature's `DEP-FRAMEWORK-AUDIT` execution dependency (which cannot begin candidate evaluation until this audit's synthesis is complete), but this feature **grants that decision no permission** and **its own completion does not depend on 004 running** — the dependency is downstream-only (Constitution Principles III, IV, VIII).
- **FR-018** The contract MUST NOT copy current milestone-specific mechanics (gate names, rotation/case identifiers, specific class names, counts, run outcomes) into this durable research contract beyond the navigational references owners already require (see `Scope Boundaries`, self-review H).
...
- **Gate record**: the durable evidence that the accepted closeout boundary was reached and the baseline the audit anchors to.

## Failure and Blocked Semantics

- **Gate unsatisfied** at execution attempt → `AUDIT — GATE NOT YET SATISFIED`, naming the unfinished closeout boundary and its owner; no mapping begins.
...
- **Baseline drift after gate open** → re-anchor; earlier findings stay attributed to their baseline and are marked superseded, never reattributed (Principle V).
...
- perform post-closeout simplification, or approve/reject/act on any existing simplification proposal (including the post-Q2 simplification proposal);
...
- **SC-007**: Execution began only after the gate record (accepted closeout evidence + anchored baseline) exists; 0 mapping/classification actions predate it.
...
- The "current accepted BeautyQ delivery closeout" this audit waits on is the closeout owned by the Q2 closeout feature (`specs/002-beautyq-q2-closeout`), whose accepted boundary set is historically documented in the post-cutover plan; this contract names its gate by role and does not freeze its milestone internals (FR-018).
...
- The post-closeout simplification decision is now materialized as `004-beautyq-post-closeout-simplification-decision`, which treats **completion of this audit as one of its execution dependencies (`DEP-FRAMEWORK-AUDIT`)**. That dependency runs downstream only: 004's existence, timing, or outcome does not gate *this* audit's completion, and this audit authorizes 004 nothing. The audit is complete on its own synthesis (Constitution Principle III: completion names its own object — the research contract, not downstream decisions).
