...
final class BeautyQProtectedRecoveryReserveSpec extends AnyWordSpec {
  ...
  private def loadLegacyFixture(
    ...
    val authorRaw = reader(AuthorReserveResource).flatMap(schema(_, "beautyq-protected-recovery-author-reserve-v7"))
    val judgedRaw = reader(JudgedReserveResource).flatMap(schema(_, "beautyq-protected-recovery-judged-reserve-v7"))
    val auditRaw = for {
      author <- authorRaw
      judged <- judgedRaw
      ...
      normalized <- parse(raw).left.map(_.message).map { json =>
        ...
        obj = obj.add("schemaVersion", io.circe.Json.fromString("beautyq-protected-recovery-selection-audit-v9"))
  ...
  "BeautyQProtectedRecoveryReserve" should {
    "load tracked resources from classpath without target dependency" in {
      BeautyQProtectedRecoveryReserve.load(classpathReader) match {
        case Right(value) =>
          assert(value.authorReserve.schemaVersion == "beautyq-protected-recovery-author-reserve-v7")
          assert(value.judgedReserve.schemaVersion == "beautyq-protected-recovery-judged-reserve-v7")
          assert(value.selectionAudit.schemaVersion == "beautyq-protected-recovery-selection-audit-v9")
          assert(value.authorReserve.authorPassId == "q2i8-recovery-rotation-8-fresh-reserve-author-v1")
          assert(value.judgedReserve.judgePassId == "q2i8-recovery-rotation-8-fresh-reserve-judge-v1")
          assert(value.selectionAudit.auditPassId == "q2i8-recovery-rotation-8-fresh-reserve-selection-audit-v1")
          assert(value.selectionAudit.authorReserveSha256 == "34eee2258894d2cdfd40c136e469f306b47d1e9fde5c7dfe03b3fd450d53344c")
          assert(value.selectionAudit.judgedReserveSha256 == "460f51e47b2ab652d59221109afe022d5782b4d0af60f575a04500baaa87364c")
    ...
    "reject changed final author query" in {
      ...
      val modifiedJson = parse(rawAuthorDraft).getOrElse(fail("invalid json")).mapObject { obj =>
        val casesArray = obj("cases").flatMap(_.asArray).getOrElse(fail("cases missing"))
        val reserveCaseIdx = casesArray.indexWhere(c => c.asObject.flatMap(_("id")).flatMap(_.asString).contains("q2i8_recovery_001"))
    ...
    "select q2i8_recovery_008 for the fresh nail-modeling bucket without an ID-specific exception" in {
      ...
      assert(selectedId == "q2i8_recovery_008")
      assert(!reserve.selectionAudit.selectedCaseIds.contains("q2i7_recovery_072"))
    }
