...
final class BeautyQGen2CutoverGateSpec extends AnyWordSpec {
  "BeautyQCutoverGate" should {
    "accept the four fixed observations when supplementing improves one query without harm" in {
      val result = BeautyQCutoverGate.evaluate(healthy)
    ...
    "reject the matrix when no query improves" in {
      val result = BeautyQCutoverGate.evaluate(healthy.map {
    ...
    "reject lost baseline IDs, order changes, owned-component changes and budget violations" in {
      ...
      val lostResult = BeautyQCutoverGate.evaluate(lost)
      ...
      val reorderedResult = BeautyQCutoverGate.evaluate(reordered)
      ...
      val ownedResult = BeautyQCutoverGate.evaluate(ownedChanged)
      ...
      val budgetResult = BeautyQCutoverGate.evaluate(budgetExceeded)
      ...
      val duplicateResult = BeautyQCutoverGate.evaluate(duplicate)
    ...
    "reject an incomplete or reordered fixed query matrix" in {
      val missing = BeautyQCutoverGate.evaluate(healthy.dropRight(1))
      assert(!missing.passed)
      assert(missing.metrics.testedQueries == 3)

      val reordered = BeautyQCutoverGate.evaluate(healthy.reverse)
    ...
    "reject a duplicated typed fixture" in {
      ...
      val result = BeautyQCutoverGate.evaluate(duplicated)
      assert(!result.passed)
