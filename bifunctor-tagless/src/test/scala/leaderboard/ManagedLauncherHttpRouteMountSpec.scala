      ...
      make[SemanticCandidateBackend[IO]].from {
        (embeddingClient: EmbeddingClient, qdrantSearchClient: QdrantSearchClient) =>
          new QdrantSemanticCandidateBackend(
            new QdrantSemanticCandidateSearch(embeddingClient, qdrantSearchClient, leaderboard.search.document.BeautyQVariantSearchDocumentContract.Fields.variantId),
            routeVectorSpec,
          )
      }
