...
import leaderboard.plugins.{BeautySearchRouteModules, ElasticsearchClientModules}
...
final class BeautySearchElasticsearchHttpRouteModuleSpec extends AnyWordSpec with HttpContractTestSupport {

  "BeautySearchRouteModules.seedCatalogElasticsearch with ElasticsearchClientModules.portConfigured" should {
    "serve POST /beauty-search through the real HTTP client module" in {
  ...
  private def buildProbe(port: Int): BeautySearchElasticsearchHttpRouteProbe = {
    val module = new distage.ModuleDef {
      ...
      make[BeautySearchElasticsearchHttpRouteProbe].from {
        (
          beautySearchApi: BeautySearchApi[IO],
          allHttpApis: Set[HttpApi[IO]],
          lifecycleMetadata: ElasticsearchSeedLifecycleMetadata,
        ) =>
          BeautySearchElasticsearchHttpRouteProbe(beautySearchApi, allHttpApis, lifecycleMetadata)
      }
    }

    val locator = Injector().produce(
      bindings = module,
      roots = Roots.target[BeautySearchElasticsearchHttpRouteProbe],
      activation = Activation.empty,
      locatorPrivacy = LocatorPrivacy.PublicByDefault,
    ).unsafeGet()

    locator.get[BeautySearchElasticsearchHttpRouteProbe]
  }
  ...
  private final case class BeautySearchElasticsearchHttpRouteProbe(
    beautySearchApi: BeautySearchApi[IO],
    allHttpApis: Set[HttpApi[IO]],
    lifecycleMetadata: ElasticsearchSeedLifecycleMetadata,
  )
