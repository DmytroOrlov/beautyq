...
object BeautySearchSpecV1 {
  ...
  private val dictionary: List[SearchSynonym] =
    List(
      ...
      SearchSynonym(tokens = Set("face"), constraints = List(enumConstraint("body_area", "face")), requires = List(SearchConstraint.ServiceAny(Set(FaceService))), excludes = List(enumConstraint("body_area", "face_neck_decollete"))),
