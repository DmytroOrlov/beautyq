...
/** M12 BeautyQ fusion/reranking experiment input scaffold contract: turns the accepted M11B result schema
  ...
  * ([[M11BeautyQSearchCandidateGenerationBoundaryFailureMatrix]]) — including their verdicts, the 63-row
...
object M12BeautyQSearchFusionRerankingInputScaffold {
  ...
  def build(): M12BeautyQSearchFusionRerankingInputScaffoldSummary = {
    ...
    // The scaffold is ready when it consumes the accepted 63-row M11B schema and the ready 20-row M11C
    ...
    val m12FusionRerankingInputScaffoldReady =
      M11BeautyQSearchCandidateGenerationResultSchema.DefaultSummary.totalRowCount == 63 &&
