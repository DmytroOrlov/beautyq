...
/** M12 BeautyQ fusion/reranking experiment-plan schema contract: a pure offline schema that consumes
  ...
  *
  * The plan preserves the M12A counts (63 input rows, 62 backend candidate placeholder rows, 0
  * executable rows, 0 real candidate result rows, 110 pending/not-executed result-leg rows, 48
  * combined comparison pair placeholders, 1 accepted negative-control exclusion) and derives
  * policy-plan counts (13 ES baseline rows, 1 Qdrant baseline row, 48 combined experiment rows, 1
...
object M12BeautyQSearchFusionRerankingExperimentPlan {
  ...
  def build(): M12BeautyQSearchFusionRerankingExperimentPlanSummary = {
    ...
    // The plan is ready when it consumes the accepted 63-row M12A scaffold, every plan row is
...
object M12BeautyQSearchFusionRerankingExperimentPlanRenderer {
  ...
  def renderMarkdown(summary: M12BeautyQSearchFusionRerankingExperimentPlanSummary): String = {
    ...
    line(builder, "experiment-plan row per consumed M12A input envelope and is NOT scoring, NOT fusion, NOT")
    line(builder, "reranking, NOT backend execution, and NOT production routing: no ES or Qdrant client is created")
