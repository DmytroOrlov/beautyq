  ...
  "BeautyQ Gen2 inbound vocabulary" should {
    ...
    "pin dynamic naming prefixes and stable rule IDs" in {
      ...
      assert(dynamicNames.forall(name => name.startsWith("attribute.int.") || name.startsWith("attribute.decimal.") || name.startsWith("attribute.enum.") || name.startsWith("attribute.boolean.")))
       assert(BeautyQIntentVocabulary.rules.map(_.id.value) == (1 to 102).map(index => f"r$index%03d").toVector)
      ...
      assert(BeautyQIntentVocabulary.rules.find(_.id.value == "r005").exists(_.aliases.contains("nail modeling extension")))
      assert(BeautyQIntentVocabulary.rules.find(_.id.value == "r070").exists(_.aliases.contains("waxing")))
       ...
       assert(BeautyQIntentVocabulary.rules.find(_.id.value == "r093").exists(_.aliases.contains("remove nail modeling")))
       ...
       assert(!r100.noise)
        ...
        assert(!r101.noise)
        val r102 = BeautyQIntentVocabulary.rules.find(_.id.value == "r102").getOrElse(fail("r102 not found"))
        assert(r102.aliases == Vector("remove gel nail modeling"))
        assert(r102.hardActions == Vector(
          service("nail_modeling"),
          BeautyIntentAction.EnumAttribute("nail_service_type", "removal"),
          BeautyIntentAction.BooleanAttribute("with_removal", true),
          BeautyIntentAction.EnumAttribute("nail_coating_type", "gel"),
        ))
        assert(r102.mode == IntentRuleMode.Independent)
        assert(r102.requires.isEmpty)
        assert(r102.semanticActions.isEmpty)
        assert(r102.excludes.isEmpty)
        assert(!r102.noise)
    }
  }
