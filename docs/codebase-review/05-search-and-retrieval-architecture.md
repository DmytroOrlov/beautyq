...
- `ElasticsearchAppStartServingGateSpec` proves app-start fail-closed behavior at composition level (blank source, empty documents, ES client failure) and DI-graph level (ES client failure prevents `BeautySearchApi` construction). Also proves prepared-serving: successful composition produces `Prepared` transition and `POST /beauty-search` returns `200 OK` when the separate `BeautySearchServingGate` (see below) is disabled.
- `ElasticsearchOperatorVisibilityEndpointPolicySpec` has zero pending tests. The canonical pending map lives in `06-tests-and-contracts.md`.
...
- The lifecycle status model/encoder is implemented and HTTP-exposed only through the explicit opt-in/internal operator endpoint `GET /ops/beauty-search/lifecycle`. That endpoint is separate from `BeautySearchServingGate`, the disabled-by-default runtime route gate for `/beauty-search` itself (implemented as of M14B: disabled -> 200 OK existing ES-backed behavior; enabled-not-ready -> HTTP 503; enabled-ready -> 200 OK; invalid request -> 400 before the gate). See `docs/BEAUTYQ_CURRENT_STATE_AND_HANDOFF.md` sections 1-2 for the full statement.
...
Until those areas are defined and implemented, the current ES-backed route remains a seed-only readiness seam with eager preparation, not a full production lifecycle. The ES post-M5 planning aggregate (startup-readiness lifecycle) is closed as docs-only planning: app-start failures fail closed by preventing route construction; successfully constructed ES route graphs are prepared-serving; stale/previous-index serving, replacement/freshness/rollback, and full lifecycle operations remain future tracks.

The startup-readiness lifecycle gate above is distinct from the separately implemented `BeautySearchServingGate` runtime route gate for `/beauty-search` (M14B): disabled (default) preserves existing ES-backed behavior, enabled-not-ready returns HTTP 503, enabled-ready preserves existing ES-backed behavior, and invalid requests still return 400 before the gate is evaluated. The ES operator visibility track is implemented separately as explicit opt-in/internal Design A: `GET /ops/beauty-search/lifecycle` is available only through `BeautySearchRouteModules.seedCatalogElasticsearchWithOperatorVisibility` / `apiElasticsearchWithOperatorVisibility`, returns `ElasticsearchStartupReadinessStatusResponse.Prepared` with nested `ElasticsearchLifecycleStatusResponse`, uses `200 OK`, requires no new ES calls, and does not change `/beauty-search`. Default `seedCatalogElasticsearch`, default `apiElasticsearch`, `LeaderboardPlugin.modules.apiBase[IO] + BeautySearchRouteModules.apiElasticsearch`, and `seedCatalogInMemory` do not expose the endpoint. See `docs/BEAUTYQ_CURRENT_STATE_AND_HANDOFF.md` sections 1-2 for the full M14-M17 statement.
...
- Existing green evidence is sufficient to keep that opt-in route as an approved disabled-by-default construction path. It is not sufficient to approve a future default `/beauty-search` switch.
...
- The production activation decision boundary is documented in `docs/codebase-review/QDRANT_PRODUCTION_ACTIVATION_DECISION_CRITERIA.md`: separate explicit approval, default graph exposure plan, rollback/disable plan, observability/status evidence, route exposure tests, and post-implementation full-suite verification are required before any default route change.
...
For the broader ES + Qdrant + hybrid retrieval end-state, see `docs/codebase-review/BEAUTYQ_ES_QDRANT_HYBRID_RETRIEVAL_ROADMAP.md`.
