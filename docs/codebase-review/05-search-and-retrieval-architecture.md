...
Until those areas are defined and implemented, the current ES-backed route remains a seed-only readiness seam with eager preparation, not a production lifecycle. The ES post-M5 planning aggregate is closed as docs-only planning: app-start failures fail closed by preventing route construction; successfully constructed ES route graphs are prepared-serving; runtime route-gate remains deferred under Candidate A; stale/previous-index serving, replacement/freshness/rollback, and full lifecycle operations remain future tracks.
...
- The production activation decision boundary is documented in `docs/local/QDRANT_PRODUCTION_ACTIVATION_DECISION_CRITERIA.md`: separate explicit approval, default graph exposure plan, rollback/disable plan, observability/status evidence, route exposure tests, and post-implementation full-suite verification are required before any default route change.
