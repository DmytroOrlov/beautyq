...
- The lifecycle status model/encoder is implemented and HTTP-exposed only through the explicit opt-in/internal operator endpoint `GET /ops/beauty-search/lifecycle`. That endpoint is separate from `BeautySearchServingGate`, the disabled-by-default runtime route gate for `/beauty-search` itself (implemented as of M14B: disabled -> 200 OK existing ES-backed behavior; enabled-not-ready -> HTTP 503; enabled-ready -> 200 OK; invalid request -> 400 before the gate). See `docs/BEAUTYQ_CURRENT_STATE_AND_HANDOFF.md` section 0 for the full statement.
