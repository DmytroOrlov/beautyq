...
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Design A is implemented and hardened as explicit opt-in/internal module: `GET /ops/beauty-search/lifecycle` is NOT in the default ES route graph; available only through `BeautySearchRouteModules.seedCatalogElasticsearchWithOperatorVisibility` / `apiElasticsearchWithOperatorVisibility`. Returns `ElasticsearchStartupReadinessStatusResponse.Prepared` with nested `ElasticsearchLifecycleStatusResponse`, uses `200 OK`, requires no new ES calls, and does not change `/beauty-search`. Design B/C, auth/config seam, local/dev fallback, and dashboard integration remain future.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_ENDPOINT_POLICY.md`: Design A operator visibility endpoint policy and hardening notes. Evidence: records the implemented path `GET /ops/beauty-search/lifecycle`, module-level opt-in/internal exposure, `200 OK` policy, `ElasticsearchStartupReadinessStatusResponse.Prepared` response shape, no-new-ES-calls behavior, and the remaining future-only policy gaps (auth/config seam, Design B/C, runtime route gate, local/dev fallback).
