...
- `docs/codebase-review/06-tests-and-contracts.md`: canonical verification-scope owner for current route/lifecycle/readiness boundaries and the canonical 8-entry pending-expectation map.
...
- `bifunctor-tagless/src/main/scala/leaderboard/search/qdrant/QdrantProductionCandidateActivationPlanning.scala`: pure M7 pre-wiring prerequisite model over M6 readiness, activation-policy readiness, config, no-regression, observability/status, rollback/disable, separate serving approval, and four future target scopes; it does not bind or enable a route.
- `bifunctor-tagless/src/main/scala/leaderboard/search/qdrant/QdrantProductionCandidateActivationConfigApproval.scala`: pure disabled-by-default config-gate and no-regression evidence/approval model; it translates only complete approved evidence into existing M7 planning inputs and has no runtime config or route behavior.
- `bifunctor-tagless/src/main/scala/leaderboard/search/qdrant/QdrantProductionCandidateObservabilityReadiness.scala`: pure readiness/status, quality/eval, and activation-decision report evidence with deterministic readiness mapping; no traffic-mirroring requirement.
- `bifunctor-tagless/src/main/scala/leaderboard/search/qdrant/QdrantProductionCandidateRollbackReadiness.scala`: pure disable-control, rollback-path, and no-regression evidence with deterministic readiness mapping; production-route activation remains unapproved.
- `bifunctor-tagless/src/test/scala/leaderboard/search/QdrantProductionCandidateM6CloseoutSpec.scala`: pure M6 source-confirmation closeout. Evidence: eight categories, conservative default, per-category blockers, adapter composition, active/all-ready success, inactive/all-ready failure, and no serving behavior.
- `bifunctor-tagless/src/test/scala/leaderboard/search/QdrantProductionCandidateM7ActivationPlanningSpec.scala`: pure M7 activation-planning source confirmation. Evidence: four target scopes, mandatory M6/policy readiness, serving-scope prerequisites, deterministic blockers, and no route implementation.
- `bifunctor-tagless/src/test/scala/leaderboard/search/QdrantProductionCandidateM7ConfigApprovalSpec.scala`: pure M7 disabled-default config/no-regression coverage, deterministic blockers, planning composition, and production-route non-approval.
- `bifunctor-tagless/src/test/scala/leaderboard/search/QdrantProductionCandidateM7CloseoutSpec.scala`: pure M7 closeout aggregate over ready M6/policy reports, conservative and approved config/no-regression evidence, explicit opt-in prerequisites, blocked production/hybrid decisions, and route-boundary spec evidence; no serving implementation or approval.
- `bifunctor-tagless/src/test/scala/leaderboard/search/BeautySearchOptInRouteModuleSpec.scala`: active in-memory opt-in route coverage plus `5` compile-safe pending Qdrant route-boundary expectations; the canonical owner/approval/activation/removal map lives in `docs/codebase-review/06-tests-and-contracts.md`.
...
- `docs/codebase-review/M5_ES_LIFECYCLE_CHECKPOINT.md`: M5 startup-readiness lifecycle closeout checkpoint. Evidence: documents bounded M5 definition, source/test evidence justifying closure, and named future ES lifecycle tracks. Also records the current post-M5 state: Design A operator visibility is explicit opt-in/internal only and hardened; default serving behavior remains unchanged. Runtime route-gate, replacement/freshness/rollback, and full lifecycle operations are intentionally separate future tracks.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Evidence: confirms the implemented files and seams, confirms no new ES calls were needed, confirms no `/beauty-search` change was needed, and documents the current active hardening coverage plus future-only gaps.
- `docs/codebase-review/ES_STARTUP_SERVING_GATE_SOURCE_CONFIRMATION.md`: source-confirmed implementation slice analysis for startup serving gate. Evidence: confirms production route construction path, smallest candidate enforcement seam (`BeautySearchApi.serverLogic`), implicit app-start fail-closed behavior, runtime route gate seam gap, and recommended deferral of runtime gate work behind Candidate A. Docs-only; no source, test, endpoint, or serving behavior changed. M5 is closed as a bounded startup-readiness lifecycle checkpoint; this source confirmation completed the bounded M5 decision.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_SOURCE_CONFIRMATION.md`: source-confirmed operator visibility track analysis. Evidence: confirms current source models (`ElasticsearchLifecycleStatusResponse`, `ElasticsearchStartupReadinessStatusResponse`, `ElasticsearchStartupReadinessTransition`, `ElasticsearchProductionReadinessState`) can be exposed without new ES calls; documents the Design A/B/C split; and records the current implemented Design A state as explicit opt-in/internal endpoint exposure while preserving future-only Design B/C work.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Design A is implemented and hardened as explicit opt-in/internal module: `GET /ops/beauty-search/lifecycle` is NOT in the default ES route graph; available only through `BeautySearchRouteModules.seedCatalogElasticsearchWithOperatorVisibility` / `apiElasticsearchWithOperatorVisibility`. Returns `ElasticsearchStartupReadinessStatusResponse.Prepared` with nested `ElasticsearchLifecycleStatusResponse`, uses `200 OK`, requires no new ES calls, and does not change `/beauty-search`. Design B/C, auth/config seam, local/dev fallback, dashboard integration, and runtime serving-gate deferral remain future.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_ENDPOINT_POLICY.md`: Design A operator visibility endpoint policy and hardening notes. Evidence: records the implemented path `GET /ops/beauty-search/lifecycle`, module-level opt-in/internal exposure, `200 OK` policy, `ElasticsearchStartupReadinessStatusResponse.Prepared` response shape, no-new-ES-calls behavior, and the remaining future-only policy gaps (auth/config seam, Design B/C, runtime route gate, local/dev fallback, runtime-gate deferral).
...
- The former MasterServiceOfferVariant typed-GET plan conflicted with current typed-GET docs and tests; it has now been removed after canonical references were updated.

### Docs pruning candidates

| File | Current role | Unique source truth | Overlap with canonical docs | Current classification | Next action |
|---|---|---|---|---|---|
| former legacy JSON single-entity GET note | Narrow inventory of the remaining repo-wide legacy raw-JSON single-entity GET boundary after Beauty typed-GET migration | No current unique source truth after this pass | Replaced by `04-api-and-http-contracts.md`, `06-tests-and-contracts.md`, `ProfileApi.scala`, `LegacyJsonResponse.scala`, `ProfileApiHttpContractSuite.scala`, and `LegacySingleEntityGetHttpContractSuite.scala` | removed | Safe to delete because canonical docs now carry the `ProfileApi` exception, `LegacyJsonResponse.optionalAsJson` helper boundary, and the route-level proof of the remaining legacy `200 + null` contract |
| former MasterServiceOfferVariant typed-GET plan file | Historical migration plan for the former last Beauty typed-GET cutover | No current unique source truth | Replaced by `04-api-and-http-contracts.md`, `06-tests-and-contracts.md`, current Tapir endpoint source, `MasterServiceOfferVariantApiHttpContractSuite.scala`, and `LegacySingleEntityGetHttpContractSuite.scala` for the remaining Profile exception | removed | Safe to delete because it was historical only, canonical references were removed or replaced, and current typed-GET behavior is covered by canonical docs/tests |
| `docs/demo/beauty-search-es-seed-demo.md` | Operator/demo runbook for the current ES-backed seed route | Yes: focused runnable demo command, suggested demo flow, and business-facing explanation of what to show | Overlaps roadmap/handoff for route status, but canonical docs do not replace the runbook/demo flow | retain | Keep as active demo handoff; only maintain wording in touched areas to match the current route composition |
| `docs/demo/beauty-search-es-seed-demo-queries.md` | Demo query inventory with selected payloads and acceptable sample variants | Yes: selected demo query set and sample acceptable variants used for business-facing smoke/demo | Overlaps `BeautySearchElasticsearchBusinessDemoSpec` coverage and roadmap references, but canonical docs do not contain this query catalog | retain | Keep as active demo inventory; maintain only small wording cleanup in touched areas |
| `docs/search-dsl-hybrid-v1-plan.md` | Future hybrid/B-lite design history and constraints | Yes: detailed hybrid policy/design history and explicit non-goals not absorbed into roadmap summaries | Overlaps `05-search-and-retrieval-architecture.md`, `07-current-gaps-and-roadmap.md`, and handoff summaries, but remains the deepest source for hybrid design history | retain | Keep as future design-history/reference doc; revisit only if roadmap and architecture docs fully absorb its still-unique policy details |
| `docs/search-dsl-qdrant-vector-backend.md` | Future Qdrant production-candidate/vector backend design history | Yes: detailed Qdrant backend design/status and non-production boundaries not fully duplicated elsewhere | Overlaps `05-search-and-retrieval-architecture.md`, `07-current-gaps-and-roadmap.md`, and architecture decisions, but those docs summarize rather than replace it | retain | Keep as future design-history/reference doc; revisit only if canonical docs absorb the remaining Qdrant-specific design details and references are removed |
...
- `LegacyJsonResponse.scala`: central legacy optional JSON response helper; current canonical docs record that it encodes `Option[A]` as `Json.Null` or entity JSON for the remaining `ProfileApi` exception.
...
- `ElasticsearchProductionReadinessState`: pure internal state derived from seed lifecycle metadata and bound through the ES route graphs. It records that serving readiness is not enforced, replacement and rollback are not configured, freshness is not tracked, refresh is eager seed preparation only, and operator visibility is not exposed in the base state. Design A operator visibility is exposed separately through the explicit opt-in/internal endpoint.
- `ElasticsearchLifecycleStatusResponse`: pure non-serving response model derived from `ElasticsearchProductionReadinessState`, with local Circe encoding for the documented current values and `productionLifecycleComplete = false`. It is not DI-bound or HTTP-exposed except through the explicit opt-in/internal Design A endpoint.
- `ElasticsearchStartupReadinessTransition`: pure, non-serving prepared/operation-failure transition model. Prepared values preserve readiness state and derive a lifecycle status response; failures retain stable operation/message data without lifecycle metadata/status response. Both record serving decision `NotEnforced`, which does not implement a serving gate. Now bound through DI via `BeautySearchCatalogBackendModules.seedResourceElasticsearch` from `ElasticsearchSeedSearchComposition.startupReadinessTransition`. App-start fail-closed and prepared-serving are test-covered by `ElasticsearchAppStartServingGateSpec`; these tests document current implicit behavior only and are not runtime HTTP 503 gate tests.
- `ElasticsearchStartupReadinessStatusResponse`: pure non-serving startup status projection from `ElasticsearchStartupReadinessTransition`. Prepared projections include nested `ElasticsearchLifecycleStatusResponse`; failed projections expose operation/message only without lifecycle metadata or status response. Local Circe encoding is provided. It is not DI-bound or HTTP-exposed except through the explicit opt-in/internal Design A endpoint. Cross-model consistency coverage exists in `ElasticsearchReadinessConsistencySpec`.
...
- `QdrantProductionCandidateActivationPlanning`: pure M7 pre-wiring prerequisite decision model; it does not bind or enable a route.
...
- `QdrantProductionCandidateObservabilityReadiness`: pure report-availability evidence and observability readiness adapter.
- `QdrantProductionCandidateRollbackReadiness`: pure disable, rollback-path, and no-regression readiness adapter.
...
- `QdrantProductionCandidateControlsReadinessSpec.scala`
- `QdrantProductionCandidateM6CloseoutSpec.scala`
- `QdrantProductionCandidateM7ActivationPlanningSpec.scala`
- `QdrantProductionCandidateM7ConfigApprovalSpec.scala`
- `QdrantProductionCandidateM7CloseoutSpec.scala`
...
- `ElasticsearchProductionReadinessState`: `search/elasticsearch/ElasticsearchProductionReadinessState.scala`; category: search/Elasticsearch readiness; responsibility: pure non-serving aggregate of current seed lifecycle metadata and explicit production lifecycle gaps. Operator-facing exposure beyond Design A remains future.
- `ElasticsearchLifecycleStatusResponse`: `search/elasticsearch/ElasticsearchLifecycleStatusResponse.scala`; category: search/Elasticsearch readiness response; responsibility: pure state-to-response projection and local Circe encoding only, with no endpoint, route path, status policy, operator policy, or DI binding.
- `ElasticsearchStartupReadinessTransition`: `search/elasticsearch/ElasticsearchStartupReadinessTransition.scala`; category: search/Elasticsearch startup readiness; responsibility: pure preparation success/source-backed operation-failure classification with `NotEnforced` serving decision, no effects, bound through DI via `BeautySearchCatalogBackendModules.seedResourceElasticsearch` from composition, and no serving behavior change.
- `ElasticsearchStartupReadinessStatusResponse`: `search/elasticsearch/ElasticsearchStartupReadinessStatusResponse.scala`; category: search/Elasticsearch startup readiness response; responsibility: pure non-serving startup status projection from prepared and failed transitions, with local Circe encoding and focused pure tests. Not DI-bound or HTTP-exposed except through the explicit opt-in/internal Design A endpoint.
...
- `QdrantProductionCandidateReadiness`, `QdrantProductionCandidateIndexingReadiness`, `QdrantProductionCandidateSearchReadiness`, `QdrantProductionCandidateQualityGate`, `QdrantProductionCandidateActivationPolicy`, `QdrantProductionCandidateActivationPlanning`, `QdrantProductionCandidateActivationConfigApproval`, `QdrantProductionCandidateObservabilityReadiness`, `QdrantProductionCandidateRollbackReadiness`: `search/qdrant/*.scala`; category: search/Qdrant production-candidate readiness and activation planning; responsibility: pure readiness/prerequisite evidence and status mapping only, without route or serving integration.
...
- Closed in this pass: `MasterServiceOfferVariantApi` typed GET migration is complete in current source/tests; canonical docs now point to `04-api-and-http-contracts.md`, `06-tests-and-contracts.md`, Tapir endpoint source, and route-level tests, and the historical plan file was removed.
