...
- `ElasticsearchStartupReadinessTransition`: pure, unbound prepared/operation-failure transition model. Prepared values preserve readiness state and derive a lifecycle status response; failures retain stable operation/message data without lifecycle metadata/status response. Both record serving decision `NotEnforced`, which does not implement a serving gate.
