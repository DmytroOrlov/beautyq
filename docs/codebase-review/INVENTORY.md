...
- `docs/search-dsl-qdrant-vector-backend.md`: Qdrant/vector backend plan/status. Evidence: states Qdrant is separate semantic/vector recall backend, complements Elasticsearch, and production hybrid is not implemented. It lists existing compatibility/indexing/search/eval pieces and the pure `QdrantProductionCandidateReadiness*` M6 foundation. The roadmap frames Qdrant as a direct production-candidate target rather than a shadow-first path.
