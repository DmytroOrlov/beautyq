...
- ES lifecycle status shape, startup serving-gate design/source-confirmation, runtime route-gate policy, M5 checkpoint, and operator-visibility source-confirmation/policy docs that previously lived under `docs/codebase-review/` were removed as superseded planning/source-confirmation leftovers once `BeautySearchServingGate` (M14B) and the `GET /ops/beauty-search/lifecycle` operator-visibility endpoint were implemented. Current truth for both is in `docs/BEAUTYQ_CURRENT_STATE_AND_HANDOFF.md` section 0.
