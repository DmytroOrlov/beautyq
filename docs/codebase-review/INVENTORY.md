...
- `docs/codebase-review/ES_LIFECYCLE_STATUS_DESIGN.md`: Elasticsearch lifecycle status shape and implementation boundary. Evidence: records the implemented non-serving response model/encoder and current values, plus the explicit opt-in/internal Design A endpoint `GET /ops/beauty-search/lifecycle`; default ES graphs do not expose it. M5 is closed as a bounded startup-readiness lifecycle checkpoint; runtime route gate and richer lifecycle policy remain future work.
...
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Design A is implemented as explicit opt-in/internal module: `GET /ops/beauty-search/lifecycle` is NOT in the default ES route graph; available only through `BeautySearchRouteModules.seedCatalogElasticsearchWithOperatorVisibility` / `apiElasticsearchWithOperatorVisibility`. Returns `ElasticsearchStartupReadinessStatusResponse.Prepared` with nested `ElasticsearchLifecycleStatusResponse`, uses `200 OK`, requires no new ES calls, and does not change `/beauty-search`. Design B/C remain future.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_ENDPOINT_POLICY.md`: Design A operator visibility endpoint policy and hardening notes. Evidence: records the implemented path `GET /ops/beauty-search/lifecycle`, module-level opt-in/internal exposure, `200 OK` policy, `ElasticsearchStartupReadinessStatusResponse.Prepared` response shape, no-new-ES-calls behavior, and the remaining future-only policy gaps (auth/config seam, Design B/C, runtime route gate).
