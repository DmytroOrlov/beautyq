...
- `docs/codebase-review/ES_LIFECYCLE_STATUS_DESIGN.md`: Elasticsearch lifecycle status shape and implementation boundary. Evidence: records the implemented non-serving response model/encoder and current values without implementing an endpoint, route path, HTTP status policy, or operator policy. M5 is closed as a bounded startup-readiness lifecycle checkpoint; endpoint exposure remains future work (ES operator visibility track).
- `docs/codebase-review/ES_STARTUP_SERVING_GATE_DESIGN.md`: ES startup serving-gate policy design. Evidence: defines five policy choices (fail closed until prepared, fail fast on preparation failure, continue serving with seed-only status, serve stale/previous index, operator override) that must be approved before any enforcement implementation. Design-only; no enforcement code, endpoint, route path, or production lifecycle completion exists. M4 is closed; M5 is closed as a bounded startup-readiness lifecycle checkpoint. Runtime route-gate work moves to ES runtime serving-gate future track.
- `docs/codebase-review/M5_ES_LIFECYCLE_CHECKPOINT.md`: M5 startup-readiness lifecycle closeout checkpoint. Evidence: documents bounded M5 definition, source/test evidence justifying closure, and named future ES lifecycle tracks. Docs-only; no source, test, endpoint, or serving behavior changed. M5 is closed as a bounded startup-readiness lifecycle checkpoint; full ES production lifecycle remains incomplete.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Evidence: confirms smallest safe implementation files, confirms no new ES calls needed, confirms no `/beauty-search` change needed, identifies DI/projection/route wiring seams, lists unresolved decisions, confirms pending specs as current expectations. Docs-only; no source, test, endpoint, or serving behavior changed.
- `docs/codebase-review/ES_STARTUP_SERVING_GATE_SOURCE_CONFIRMATION.md`: source-confirmed implementation slice analysis for startup serving gate. Evidence: confirms production route construction path, smallest candidate enforcement seam (`BeautySearchApi.serverLogic`), implicit app-start fail-closed behavior, runtime route gate seam gap, and recommended spec-only route-level tests before enforcement. Docs-only; no source, test, endpoint, or serving behavior changed. M5 is closed as a bounded startup-readiness lifecycle checkpoint; this source confirmation completed the bounded M5 decision.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_SOURCE_CONFIRMATION.md`: source-confirmed operator visibility track analysis. Evidence: confirms current source models (`ElasticsearchLifecycleStatusResponse`, `ElasticsearchStartupReadinessStatusResponse`, `ElasticsearchStartupReadinessTransition`, `ElasticsearchProductionReadinessState`) can be exposed without new ES calls; identifies exposable vs non-exposable data; identifies candidate endpoint seam following existing Tapir/http4s patterns; documents three operator visibility designs (A: prepared-only status, B: startup failure via bootstrap state, C: richer status after replacement/freshness/rollback); recommends Design A as next step; lists policy decisions required before endpoint implementation. Docs-only; no source, test, endpoint, or serving behavior changed. M5 is closed as a bounded startup-readiness lifecycle checkpoint; operator visibility is the next source-confirmed track.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`: implementation-slice source confirmation for Design A operator visibility. Design A is now implemented as explicit opt-in module: `GET /ops/beauty-search/lifecycle` endpoint is NOT in the default ES route graph; available only through `BeautySearchRouteModules.apiElasticsearchWithOperatorVisibility`. Returns `ElasticsearchStartupReadinessStatusResponse.Prepared` with nested `ElasticsearchLifecycleStatusResponse`, uses `200 OK`, requires no new ES calls, and does not change `/beauty-search`. Files created: `EsLifecycleStatusTapirEndpoints.scala`, `EsLifecycleStatusApi.scala`. Files modified: `BeautySearchPluginModules.scala`, `BeautySearchRouteModules.scala`, `ElasticsearchStartupReadinessStatusResponse.scala`, `ElasticsearchLifecycleStatusResponse.scala`. Tests: `ElasticsearchOperatorVisibilityEndpointPolicySpec` (59 active, 5 pending). Design B/C remain future.
- `docs/codebase-review/ES_OPERATOR_VISIBILITY_ENDPOINT_POLICY.md`: Design A operator visibility endpoint policy draft. Evidence: drafts endpoint path candidates (4 candidates), auth/exposure policy options (4 options), HTTP status code policy options (3 options), and response shape candidates (3 candidates) for Design A. Draft recommends `GET /ops/beauty-search/lifecycle`, disabled-by-default auth, `200 OK` status, `ElasticsearchStartupReadinessStatusResponse` response shape. Lists implementation prerequisites, future tests, and future implementation seams. Docs-only; no source, test, endpoint, or serving behavior changed. All draft recommendations remain unapproved. Draft endpoint expectations are now captured in `ElasticsearchOperatorVisibilityEndpointPolicySpec.scala` (28 pending tests). Implementation-slice source-confirmed in `ES_OPERATOR_VISIBILITY_IMPLEMENTATION_SOURCE_CONFIRMATION.md`.
...
- `ElasticsearchProductionReadinessState`: pure internal state derived from seed lifecycle metadata and bound through the ES route graphs. It records that serving readiness is not enforced, replacement and rollback are not configured, freshness is not tracked, refresh is eager seed preparation only, and operator visibility is not exposed.
- `ElasticsearchLifecycleStatusResponse`: pure non-serving response model derived from `ElasticsearchProductionReadinessState`, with local Circe encoding for the documented current values and `productionLifecycleComplete = false`. It is not DI-bound or HTTP-exposed.
...
- `ElasticsearchStartupReadinessStatusResponse`: pure non-serving startup status projection from `ElasticsearchStartupReadinessTransition`. Prepared projections include nested `ElasticsearchLifecycleStatusResponse`; failed projections expose operation/message only without lifecycle metadata or status response. Local Circe encoding is provided. It is not DI-bound or HTTP-exposed. Cross-model consistency coverage exists in `ElasticsearchReadinessConsistencySpec`.
...
- `ElasticsearchProductionReadinessState`: `search/elasticsearch/ElasticsearchProductionReadinessState.scala`; category: search/Elasticsearch readiness; responsibility: pure non-serving aggregate of current seed lifecycle metadata and explicit production lifecycle gaps. Operator-facing exposure remains unapproved and unimplemented.
