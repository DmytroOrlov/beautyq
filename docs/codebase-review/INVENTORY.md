...
- `docs/codebase-review/ES_LIFECYCLE_STATUS_DESIGN.md`: design-only future operator-facing Elasticsearch lifecycle status shape. Evidence: defines the planned response fields and current documented values without implementing an endpoint, route path, or operator policy.
...
- `ElasticsearchProductionReadinessState`: pure internal state derived from seed lifecycle metadata and bound through the ES route graphs. It records that serving readiness is not enforced, replacement and rollback are not configured, freshness is not tracked, refresh is eager seed preparation only, and operator visibility is not exposed. The planned operator-facing response shape is documented separately in `docs/codebase-review/ES_LIFECYCLE_STATUS_DESIGN.md`; that design is not implemented.
...
- `ElasticsearchProductionReadinessState`: `search/elasticsearch/ElasticsearchProductionReadinessState.scala`; category: search/Elasticsearch readiness; responsibility: pure non-serving aggregate of current seed lifecycle metadata and explicit production lifecycle gaps. Any future operator-facing status surface remains design-only until separately approved and implemented.
