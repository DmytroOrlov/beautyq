...
- `bifunctor-tagless/src/test/scala/leaderboard/search/BeautySearchOptInRouteModuleSpec.scala`: active in-memory opt-in route coverage plus compile-safe pending Qdrant route-boundary expectations; the pending boundary stays outside default `apiElasticsearch` and consumes the disabled-default config/no-regression report without adding Qdrant serving.
