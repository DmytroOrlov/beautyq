...
|---|---|---|---|---|---|
| `docs/http-legacy-json-contracts.md` | Narrow inventory of the remaining repo-wide legacy raw-JSON single-entity GET boundary after Beauty typed-GET migration | Yes: it is the clearest current note that `LegacyJsonResponse.optionalAsJson` still exists and that `ProfileApi` remains intentionally outside the Beauty typed-GET inventory | Overlaps `04-api-and-http-contracts.md` and `06-tests-and-contracts.md` for Beauty single-entity GET outcomes, but those canonical docs do not yet carry the Profile/legacy-helper exception | stale but retained until a specific source/test gap is closed | Keep for now; later merge or reduce after canonical docs explicitly cover the remaining `ProfileApi` legacy path and `LegacyJsonResponse` helper boundary |
| `docs/http-master-service-offer-variant-typed-get-plan.md` | Historical migration plan for the former last Beauty typed-GET cutover | No current source truth beyond historical context; current source/tests already show typed GET is complete | Overlaps and conflicts with `04-api-and-http-contracts.md`, `06-tests-and-contracts.md`, current Tapir endpoint source, and `MasterServiceOfferVariantApiHttpContractSuite.scala` | delete candidate | Do not delete in this patch because references remain; remove canonical references or replace them with a short historical note, then delete after grep confirms no remaining links |
