...
| `M19DualEngineOfflineEvalEvidenceReport.scala` (M19B) | `reporting_or_projection` | Readable evidence-report artifact over M19A metrics. | `M19DualEngineOfflineEvalEvidenceReportSpec` | No | After consolidation | Merge with M19H into one offline-eval evidence report |
| `M19HBeautyQComponentCoverageEvidenceReport.scala` | `reporting_or_projection` | Coverage evidence over populated component expectations. | `M19HBeautyQComponentCoverageEvidenceReportSpec` | No | After consolidation | Overlaps M19C taxonomy + M19D gap model; consolidate |
...
- `M19DualEngineOfflineEvalMetricsSpec`, `M19DualEngineOfflineEvalEvidenceReportSpec`, `M19CBeautyQResponseComponentTaxonomySpec`, `M19DBeautyQCoverageGapModelSpec`, `M19FBeautyQEvalComponentExpectationSchemaSpec`, `M19HBeautyQComponentCoverageEvidenceReportSpec`.
...
3. **`M19DualEngineOfflineEvalEvidenceReport.scala` (M19B)** ↔ **`M19HBeautyQComponentCoverageEvidenceReport.scala`** — overlapping offline-eval evidence reports. *Replace:* merge into one evidence report over M19A metrics + component coverage.
4. **`M19HBeautyQComponentCoverageEvidenceReport.scala`** — coverage evidence overlaps M19C taxonomy + M19D gap model. *Replace:* single coverage projection.
...
> None deleted in this task (forbidden). Each is a candidate **only after** the named replacement owns its unique assertions.
