...
- `leaderboard.search.eval.M9BeautyQSearchEvalQueryDatasetStaticRows` maps representative BeautyQ eval fixture anchors into M9 static/saved-report rows for `q_nails_001`, `q_nails_003`, and `q_noise_005`. It uses manual/unknown-serving fixture rows, validates bounded resource text anchors instead of parsing full JSON, and feeds `M9OfflineEvalStaticRunner`; full 63-query row expansion remains future work. Checked-in artifact: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`.
- `leaderboard.search.M9BeautyQSearchEvalQueryDatasetStaticRowsSpec` locks representative RU/DE/noise ids, dataset metadata notes/summary, deterministic row count, static-runner compatibility, byte-for-byte artifact stability, no real backend call requirement, no route/plugin/DI/http source involvement, and the explicit deferral of full JSON parsing/full 63-query row expansion.
...
The uploaded 63-query BeautyQ seed-ready eval dataset is available as a checked-in test resource. Representative static rows now map selected fixture anchors into the M9 saved-report shape without real backend calls. This remains representative subset coverage only; full JSON parsing and full 63-query row expansion remain future work. It is dataset evidence for offline evaluation only: real backend calls stay disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
