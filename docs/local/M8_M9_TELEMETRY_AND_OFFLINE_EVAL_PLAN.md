...
- `leaderboard.search.eval.M9BeautyQSearchEvalQueryDataset` registers the uploaded 63-query `wandsbek_hamburg_beauty_services_seed_ready` eval dataset as a deterministic offline-eval/test-resource fixture at `leaderboard/search/eval/beautyq_search_eval_queries_v1.json`. Metadata records version, query count, language counts, target carousels, service/attribute coverage, Wandsbek Markt location label, and the boundary that this is not production telemetry or activation approval.
- `leaderboard.search.M9BeautyQSearchEvalQueryDatasetSpec` locks resource presence, dataset/version/query-count anchors, target carousel ids, language counts, service/attribute coverage anchors, representative multilingual query ids, acceptable/forbidden evidence anchors, and the offline-only/no-route-plugin-DI-http boundary. Full JSON parsing remains future work; no new JSON dependency is added.
- `leaderboard.search.eval.M9BeautyQSearchEvalQueryDatasetStaticRows` maps all 63 BeautyQ eval fixture query ids into M9 static/saved-report rows. It uses manual/unknown-serving fixture rows, validates the checked-in static id list against bounded resource anchors instead of parsing full JSON, preserves the stronger `q_nails_001`, `q_nails_003`, and `q_noise_005` top-k fixture anchors, feeds `M9OfflineEvalStaticRunner`, and keeps the remaining rows as static placeholders only. Checked-in artifact: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`.
- `leaderboard.search.M9BeautyQSearchEvalQueryDatasetStaticRowsSpec` locks the 63-row count, resource-id validation, representative RU/DE/noise ids and fixture anchors, dataset metadata notes/summary, static-runner compatibility, byte-for-byte artifact stability, no real backend call requirement, no route/plugin/DI/http source involvement, implemented full 63-query static-row expansion, and explicit deferral of full JSON parsing.
- `leaderboard.search.eval.M9BeautyQSearchEvalStaticScorecard` defines a pure static scorecard over the 63 BeautyQ static rows. It reports dataset/static readiness only: 63 dataset queries, 63 mapped rows, 63 static-runner accepted rows, 3 representative anchor rows, 60 placeholder-only rows, manual/static source only, unknown serving mode only, zero real/ES/Qdrant backend evidence rows, no production activation approval, no route/plugin/DI/http involvement, deferred full JSON parsing, and implemented full 63-query static expansion. Checked-in artifact: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-static-scorecard.md`.
- `leaderboard.search.M9BeautyQSearchEvalStaticScorecardSpec` locks all scorecard counts, attribution, zero backend evidence, dataset/static-only verdict wording, forbidden production/Qdrant/hybrid/quality/activation claims, byte-for-byte artifact stability, no real backend call requirement, no route/plugin/DI/http source involvement, and no full JSON parser requirement.
...
The uploaded 63-query BeautyQ seed-ready eval dataset is available as a checked-in test resource. Static rows now map all 63 query ids into the M9 saved-report shape without real backend calls. A static scorecard records dataset/static readiness over those rows, including 3 representative anchors, 60 placeholder-only rows, manual/unknown-only attribution, and zero backend evidence rows. Full JSON parsing remains intentionally deferred: the current slice uses a checked-in static id list validated against the resource, preserves stronger fixture anchors for `q_nails_001`, `q_nails_003`, and `q_noise_005`, and treats the remaining rows as static placeholders only. It is dataset evidence for offline evaluation only, not backend quality or activation approval: real backend calls stay disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
