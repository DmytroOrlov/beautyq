...
The uploaded 63-query BeautyQ seed-ready eval dataset is available as a checked-in test resource. Static rows now map all 63 query ids into the M9 saved-report shape without real backend calls. Full JSON parsing remains intentionally deferred: the current slice uses a checked-in static id list validated against the resource, preserves stronger fixture anchors for `q_nails_001`, `q_nails_003`, and `q_noise_005`, and treats the remaining rows as static placeholders only. It is dataset evidence for offline evaluation only: real backend calls stay disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
