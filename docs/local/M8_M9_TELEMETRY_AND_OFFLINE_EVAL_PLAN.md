...
The uploaded 63-query BeautyQ seed-ready eval dataset is available as a checked-in test resource. It is dataset evidence for offline evaluation only: real backend calls stay disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
