...
* ES-only gated smoke artifacts such as `m9-es-only-gated-smoke-not-configured-report.md` or `m9-es-only-gated-smoke-default-disabled-report.md`, generated by `M9OfflineEvalEsOnlyGatedSmokeEvidence` when no real ES resource is available
* Qdrant-only gated smoke artifacts such as `m9-qdrant-only-gated-smoke-not-configured-report.md` or `m9-qdrant-only-gated-smoke-default-disabled-report.md`, generated by `M9OfflineEvalQdrantOnlyGatedSmokeEvidence` when no real Qdrant resource is available
* Combined ES+Qdrant gated smoke evidence index artifacts such as `m9-combined-gated-smoke-evidence-index.md` or `m9-combined-gated-smoke-evidence-index-default-disabled.md`, generated by `M9OfflineEvalCombinedGatedSmokeEvidenceIndex` as an offline-eval reporting/index layer summarizing the accepted ES-only and Qdrant-only skipped/not-configured artifacts

Use `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-static-example-report.md` as the canonical format example for future saved M9 markdown evidence. It is an example-only static fixture artifact with `sample_not_for_activation`; current backend/eval runs and existing `EngineEvalAggregateReport` JSON artifacts remain separate. Use `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-es-only-gated-smoke-not-configured-report.md` as the canonical ES-only not-configured smoke artifact example. Use `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-qdrant-only-gated-smoke-not-configured-report.md` as the canonical Qdrant-only not-configured smoke artifact example.
