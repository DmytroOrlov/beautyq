...
* M6 Qdrant production-candidate readiness now has four accepted source-backed pure chunks: the readiness foundation, quality/parity gate, activation-policy foundation, and indexing/search readiness adapters. It tracks collection/identity, contract parity, indexing, search, quality/eval, observability, rollback/disable, and activation-policy readiness without route serving.
...
* Configured operational controls, accepted broader evidence, and any serving approval remain future M6 work. Rough M6 remaining after the indexing/search adapter chunk is 2–4 large commits; rough remaining to M8/project closeout is 15–20 large commits, depending on activation and hybrid decisions. M7 hybrid policy and M8 controlled hybrid serving remain future-only and conditional on completed direct Qdrant-candidate readiness plus explicit serving policy approval. Runtime serving-gate work stays deferred until a runtime readiness source or replacement/freshness/rollback policy exists.
* `seedCatalogInMemory` remains available as rollback/non-default.
...
* M6 now has pure source-backed readiness, quality/parity, activation-policy, and indexing/search adapter foundations. The conservative default is not ready; activation remains `NotApproved` unless an explicit non-production-route scope and all required controls are satisfied.
