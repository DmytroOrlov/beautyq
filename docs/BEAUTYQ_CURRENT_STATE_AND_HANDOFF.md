...
* M6 Qdrant production-candidate readiness now has a source-backed pure foundation: `QdrantProductionCandidateReadiness*`, the quality/parity gate, and `QdrantProductionCandidateActivationPolicy*`. It tracks collection/identity, contract parity, indexing, search, quality/eval, observability, rollback/disable, and activation-policy readiness without route serving.
...
* Configured operational controls, accepted evidence, and any serving approval remain future M6 work. M7 hybrid policy and M8 controlled hybrid serving remain future-only and conditional on completed direct Qdrant-candidate readiness plus explicit serving policy approval. Runtime serving-gate work stays deferred until a runtime readiness source or replacement/freshness/rollback policy exists.
* `seedCatalogInMemory` remains available as rollback/non-default.
...
* M6 now has pure source-backed readiness, quality/parity, and activation-policy foundations. The conservative default is not ready; activation remains `NotApproved` unless an explicit non-production-route scope and all required controls are satisfied.
