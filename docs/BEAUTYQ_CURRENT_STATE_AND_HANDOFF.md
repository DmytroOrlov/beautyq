...
## 5. Current priority: M7 activation/source-confirmation and serving-policy planning
...
* Pending route expectations require any future explicit opt-in Qdrant route to remain outside default `apiElasticsearch` and to require M6 readiness, activation-policy approval, a disabled-by-default config gate, no-regression evidence, observability/status evidence, and rollback/disable control. `/beauty-search` remains ES-backed until separate production-route activation approval.
* Coordinator progress snapshot: current milestone is M7 activation/source-confirmation and serving-policy planning; maximum mapped milestone is M8; accepted M6 code chunks are 5 large commits plus closeout; M6 implementation remaining is 0; rough M7 remaining after this chunk is about 3–5 large commits before any serving implementation decision; rough remaining to M8/project closeout is about 15–20 large commits, depending on activation and hybrid decisions.
...
* Recommended next large chunk: source-confirm the pending explicit opt-in module contract against the pure config/no-regression report, without route wiring.
* `seedCatalogInMemory` remains available as rollback/non-default.
...
* Shadow serving and production traffic mirroring are not readiness prerequisites. M7 activation/source-confirmation and serving-policy planning has started without serving implementation. M8 controlled hybrid serving remains future-only and conditional on separate serving approval.
