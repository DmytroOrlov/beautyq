...
* Future M8 production-telemetry planning and M9 offline-eval-harness planning are paired in `docs/local/M8_M9_TELEMETRY_AND_OFFLINE_EVAL_PLAN.md`; pure shared vocabulary/reporting contracts exist in `leaderboard.search.eval.M8M9EvalContracts`, the pure M8 telemetry schema renderer/static adapter exists in `leaderboard.search.eval.M8TelemetrySchemaRenderer`, the pure M9 saved dataset/report format plus deterministic markdown renderer exists in `leaderboard.search.eval.M9OfflineEvalSavedReport`, the pure static/in-memory M9 runner skeleton exists in `leaderboard.search.eval.M9OfflineEvalStaticRunner`, canonical static fixtures plus a checked-in example markdown artifact exist in `leaderboard.search.eval.M9OfflineEvalStaticFixtures`, first pure backend-runner interface contracts exist in `leaderboard.search.eval.M9OfflineEvalBackendRunner`, pure ES/Qdrant offline adapter skeletons exist in `leaderboard.search.eval.M9OfflineEvalRealBackendAdapterSkeleton`, and pure backend-adapter failure-matrix fixtures exist in `leaderboard.search.eval.M9OfflineEvalBackendAdapterFailureMatrix`, but telemetry emission, metrics client integration, real ES/Qdrant backend runners, route changes, hybrid serving, fusion, reranking, and production activation remain unimplemented.
* The M9 backend-runner execution seam is implemented only as pure interfaces/value contracts plus a manual/static adapter; it does not approve any production route or activation change.
...
* ES-only gated smoke evidence now exists as `M9OfflineEvalEsOnlyGatedSmokeEvidence`: deterministic ES-only smoke artifact capture reusing the spike artifact path, with ES source/mode attribution, production non-approval, no fake success, and byte-for-byte stable checked-in not-configured artifact. Real ES calls remain unimplemented and disabled by default; no route/plugin/DI/http source changed.
* Qdrant-only gated smoke evidence now exists as `M9OfflineEvalQdrantOnlyGatedSmokeEvidence`: deterministic Qdrant-only smoke artifact capture reusing the spike artifact path, with Qdrant source/mode attribution, production non-approval, no fake success, embedding/vector prerequisite absence as warning data, and byte-for-byte stable checked-in not-configured artifact. Real Qdrant calls remain unimplemented and disabled by default; no route/plugin/DI/http source changed.
* Combined ES/Qdrant gated smoke evidence index now exists as `M9OfflineEvalCombinedGatedSmokeEvidenceIndex` plus `M9OfflineEvalCombinedGatedSmokeEvidenceRenderer`: a deterministic offline-eval reporting/index layer that summarizes the accepted ES-only and Qdrant-only skipped/not-configured artifacts, preserves ES and Qdrant source/mode attribution, records both gate decisions and resource-config absence, records production activation non-approval, records that no real backend call is implemented, treats warnings/failures as data, and points to the checked-in per-backend artifacts by file name. No hybrid/fallback/fusion/reranking is claimed; no route/plugin/DI/http source changed; byte-for-byte stable checked-in combined index artifact exists.
* The uploaded 63-query seed-ready BeautyQ eval dataset is registered as `leaderboard/search/eval/beautyq_search_eval_queries_v1.json`, with pure metadata in `M9BeautyQSearchEvalQueryDataset`. It is a test-resource/offline-eval fixture only: real backend calls remain disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
* M9 BeautyQ eval dataset static-row mapping now exists as `M9BeautyQSearchEvalQueryDatasetStaticRows`: all 63 query ids from the fixture map into manual/unknown-serving M9 static/saved-report rows without full JSON parsing or backend execution. The checked-in artifact is `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`; representative RU/DE/noise rows keep their stronger top-k fixture anchors, and the remaining rows are static placeholders only. Real backend calls remain disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
* M9 BeautyQ eval static scorecard now exists as `M9BeautyQSearchEvalStaticScorecard`: it reports dataset/static readiness over the 63 manual/unknown static rows, including 3 representative anchor rows, 60 placeholder-only rows, zero ES/Qdrant/backend evidence rows, no route/plugin/DI/HTTP involvement, deferred full JSON parsing, and implemented full 63-query static expansion. The checked-in artifact is `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-static-scorecard.md`. This is not backend quality evidence or activation approval; real backend calls remain disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
...
* Pure M9 backend-runner interface/value-contract slice and manual/static adapter — implemented.
* Pure M8 telemetry schema renderer/static adapter — implemented.
...
* M9 BeautyQ search eval query dataset fixture — implemented: uploaded 63-query seed-ready dataset registered as a deterministic test resource with pure metadata/index constants. This is offline eval evidence only; default `/beauty-search` remains ES-backed and Qdrant production activation remains not approved.
* M9 BeautyQ eval dataset static rows — implemented for all 63 query ids as manual/unknown-serving fixture rows. `q_nails_001`, `q_nails_003`, and `q_noise_005` keep their stronger top-k fixture anchors; the other rows are static placeholders only. Full JSON parsing remains intentionally deferred; real backend calls remain disabled by default; default `/beauty-search` remains ES-backed; Qdrant production activation remains not approved. Artifact path: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`.
* M9 BeautyQ eval static scorecard — implemented as a dataset/static readiness contract over the 63 static rows. It records 63 dataset queries, 63 mapped/accepted static rows, 3 representative anchor rows, 60 placeholder-only rows, manual/static and unknown-serving attribution only, zero backend evidence rows, no route/plugin/DI/HTTP involvement, no production activation approval, deferred full JSON parsing, and implemented full static expansion. Artifact path: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-static-scorecard.md`.
* M9 BeautyQ real-call checkpoint contract — implemented as a pure eligibility decision over the static scorecard readiness, ES/Qdrant resource config presence, and operator approval. The default decision blocks without explicit resource config (`not_eligible_no_explicit_resource_config`); eligible decisions are offline/resource-gated only (ES-only, Qdrant-only, or both with operator approval). A missing or non-ready static scorecard blocks. Production activation remains not approved and is never output as ready; no real backend call is implemented or required; no route/plugin/DI/HTTP source is involved. Default `/beauty-search` remains ES-backed; real backend calls remain disabled by default unless an explicit future resource-smoke task runs them. Byte-for-byte checked-in artifact path: `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-real-call-checkpoint.md`.
