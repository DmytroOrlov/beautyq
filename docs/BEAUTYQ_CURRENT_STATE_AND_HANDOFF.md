...
* Qdrant explicit opt-in (`BeautySearchRouteModules.apiQdrantExplicitOptIn`) is implemented but disabled by default and lives outside the default `apiElasticsearch` graph. Qdrant is **not approved** for the default route.
* For local operator preflight/smoke checks on the no-worsening Qdrant supplement activation selector, see `docs/BEAUTYQ_QDRANT_SUPPLEMENT_LOCAL_ACTIVATION_RUNBOOK.md` and the short checklist at `docs/BEAUTYQ_QDRANT_SUPPLEMENT_OPERATOR_CHECKLIST.md`.
...
* **Accepted final checkpoint:** `sbt test` ran `1693` tests, `1693` succeeded, `0` failed, `0` aborted, `1` canceled, `0` pending. Every earlier full-suite snapshot is historical and superseded by this checkpoint.
* **Accepted QP24 stop-state:** BeautyQ Qdrant supplement work is stopped at QP24 (`805edaa48724e16a36dca2ab8955f11c52cef3b2`). Current local/test gate remains `testedQueries=4`, `improvedQueries=1`, `unchangedQueries=3`, `worsenedQueries=0`, `totalQdrantOnlyAppends=1`, `duplicateEsIds=0`, `lostEsIds=0`, `prefixOrderRegressions=0`, `esOwnedComponentChanges=0`, `appendBudgetViolations=0`. ES remains primary/default; Qdrant remains constrained supplement only.
* All formerly-pending test expectations (`BeautySearchOptInRouteModuleSpec`, `BeautySearchProductionRouteExposureSpec`, `ElasticsearchOperatorVisibilityEndpointPolicySpec`) are now active tests.
...
| M13 | Controlled opt-in route/module planning closed; preserved the ES default and explicitly denied wider route enablement. |
...
| Accepted checkpoint | Recorded the final accepted full-suite checkpoint (see section 1). |

The explicit opt-in Qdrant route remains a local/dev/test-only controlled surface. Use `docs/BEAUTYQ_QDRANT_SUPPLEMENT_OPERATOR_CHECKLIST.md` for the current local/test gate and non-goals, and `docs/SEARCH_SUPPLEMENT_FUTURE_DOMAIN_GATE_TEMPLATE.md` for the reusable supplement-gate method.
...
* Keep request/response examples and regression detail in source and focused specs; this handoff records only the stable request boundary.
...
* Use this handoff plus the current operator checklist/runbook as the active documentation surface. Older eval/roadmap/report layers were removed from the docs tree.
...
This method, not the BeautyQ threshold values, should transfer to future domains. This is anti-noise guidance: a future domain should start from this method rather than recreating a long milestone/report chain from scratch.
...
* `docs/SEARCH_SUPPLEMENT_FUTURE_DOMAIN_GATE_TEMPLATE.md` — reusable supplement gate method
* `docs/BEAUTYQ_QDRANT_SUPPLEMENT_OPERATOR_CHECKLIST.md` — current local/test gate and non-goals
* `docs/BEAUTYQ_QDRANT_SUPPLEMENT_LOCAL_ACTIVATION_RUNBOOK.md` — local activation procedure
