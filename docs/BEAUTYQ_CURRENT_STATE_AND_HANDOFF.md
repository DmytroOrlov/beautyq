...
* M9 BeautyQ eval dataset static-row mapping now exists as `M9BeautyQSearchEvalQueryDatasetStaticRows`: representative RU/DE/noise query rows map the 63-query fixture metadata into M9 static/saved-report rows without JSON parsing or backend execution. The checked-in artifact is `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`; full 63-query row expansion remains future work. Real backend calls remain disabled by default, default `/beauty-search` remains ES-backed, and Qdrant production activation remains not approved.
...
* M9 BeautyQ eval dataset static rows — implemented as representative subset mapping only: `q_nails_001`, `q_nails_003`, and `q_noise_005` feed the static saved-report runner as manual/unknown-serving fixture rows. Full JSON parsing and full 63-query row expansion remain future work; artifact path is `bifunctor-tagless/src/test/resources/leaderboard/search/eval/m9-beautyq-eval-dataset-static-rows-report.md`.
