...
* `ElasticsearchStartupReadinessTransition` is a pure, unbound startup-preparation result model. It represents prepared state or source-backed `QueryFailure.OperationFailure` details and records `ElasticsearchStartupServingDecision.NotEnforced`; it does not gate serving or change preparation/composition behavior.
