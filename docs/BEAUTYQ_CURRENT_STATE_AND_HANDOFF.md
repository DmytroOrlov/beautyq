...
* M6 Qdrant production-candidate readiness now has a source-backed pure foundation: `QdrantProductionCandidateReadiness*` plus `QdrantProductionCandidateQualityGate`, `QdrantProductionCandidateQualityReport`, `QdrantProductionCandidateParityReport`, and `QdrantProductionCandidateQualityDecision`. It tracks collection/identity, contract parity, indexing, search, quality/eval, observability, rollback/disable, and activation-policy readiness without route serving.
...
* Configured observability, rollback/disable controls, and activation approval remain future M6 work. M7 hybrid policy and M8 controlled hybrid serving remain future-only and conditional on completed direct Qdrant-candidate readiness plus explicit serving policy approval. Runtime serving-gate work stays deferred until a runtime readiness source or replacement/freshness/rollback policy exists.
* `seedCatalogInMemory` remains available as rollback/non-default.
...
* M6 now has a pure source-backed production-candidate readiness and quality/parity gate foundation. The conservative default is not ready; quality/eval becomes ready only from a passed explicit report, while activation remains unsatisfied until explicit approval exists.
