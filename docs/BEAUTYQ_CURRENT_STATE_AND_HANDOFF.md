...
* **M15A serving-readiness operational/manual smoke checklist — docs-only planning, not production activation and not serving approval.** The accepted M14 gate states and local/dev-only fallback boundary are packaged into a manual preflight, smoke-case table, boundary-check table, and evidence-field template in `docs/local/BEAUTYQ_M15_SERVING_READINESS_MANUAL_SMOKE_CHECKLIST.md`. M15A adds no source, test, resource, route/plugin/DI/HTTP, or backend-client change; it does not approve a default route switch, Qdrant activation, production fallback, runtime serving fallback by default, hybrid/fusion/reranking/telemetry behavior, or any production-readiness, route-activation, or serving-approval claim.
...
* `docs/local/BEAUTYQ_M15_SERVING_READINESS_MANUAL_SMOKE_CHECKLIST.md` — M15A operator/developer manual smoke checklist for the accepted M14 route-gate and local/dev-only fallback state
