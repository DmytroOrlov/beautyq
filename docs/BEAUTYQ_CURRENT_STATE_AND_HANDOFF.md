...
* This M7 source-confirmation chunk records the pending explicit opt-in module contract against the pure config/no-regression report without route wiring. A later implementation decision still requires separate approval.
* `seedCatalogInMemory` remains available as rollback/non-default.
