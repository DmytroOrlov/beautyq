...
| Module | Owns (current) |
|---|---|
| `leaderboard-core` | generic failure types such as `QueryFailure`; pure Elasticsearch port config DTO (`ElasticsearchPortCfg`) |
...
| `app-http` | HTTP/API/Tapir shell: `HttpApi`, `HttpApiFailure`, `BeautySearchGen2Api`, `CategoryApi`/`LadderApi`/`MasterApi`/`MasterLocationApi`/`MasterServiceOfferApi`/`MasterServiceOfferVariantApi`/`ServiceApi`/`ProfileApi`, `BeautySearchGen2TapirEndpoints`, and all `leaderboard.http.tapir.*TapirEndpoints` |
| `leaderboard-app-shell` | The app/shell module over the `beautyq-search-gen2-wiring`, `app-http`, and `app-services` boundaries. `HttpServer` (lifecycle/startup, combines `Set[HttpApi[F]]`), startup/plugin wiring (`BeautySearchGen2PluginModules`, `BeautyQSearchGen2Startup`, `BeautyQSearchGen2HttpService`, `BeautyQGen2EmbeddingClient`), config binding modules, one managed Qdrant Gen2 resource (`QdrantGen2DockerPlugin`), and the retained `BeautySearchGen2PluginModules.api` composition. It no longer owns `ProfileApi` or `Ranks` source, but still wires/consumes both via `LeaderboardPlugin`/`LeaderboardRole`. |
