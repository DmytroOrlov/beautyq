...
Indexing status is backed by `QdrantProductionCandidateIndexingReadiness`. Its pure adapter accepts the existing `QdrantSnapshotIndexingResult`; the evidence records expected document count, prepared/indexable document count, optional indexed document count, collection-identity readiness, and embedding/vector readiness. Expected and prepared counts must be positive, prepared and indexed counts must match expected, collection identity must be `Ready`, and embedding/vector evidence must be present. Missing reports map to `Unknown`; incomplete evidence maps to `NotReady` with deterministic ordered reasons.

Search status is backed by `QdrantProductionCandidateSearchReadiness`. Its pure evidence records the presence of the semantic candidate backend and search contracts, candidate assembly readiness, response projection readiness, and BeautySearch contract-parity evidence. Every field must be ready/present. Missing reports map to `Unknown`; incomplete evidence maps to `NotReady` with deterministic ordered reasons.

Observability status is backed by `QdrantProductionCandidateObservabilityReadiness`. Its pure evidence records whether readiness/status, quality/eval, and activation-decision reports exist. All three reports are required. Missing reports map to `NotConfigured`; incomplete evidence maps to `NotReady` with deterministic ordered reasons; complete evidence maps to `Ready`. Shadow telemetry and production traffic mirroring are not required.

Rollback/disable status is backed by `QdrantProductionCandidateRollbackReadiness`. Its pure evidence records whether a disable control, rollback path, and no-regression evidence are available. All three are required. Missing reports map to `NotConfigured`; incomplete evidence maps to `NotReady` with deterministic ordered reasons; complete evidence maps to `Ready`. The rollback decision explicitly records that production-route activation is not approved.
