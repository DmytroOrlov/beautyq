...
final class M9BeautyQSearchEvalQueryDatasetStaticRowsSpec extends AnyWordSpec {
  ...
  "M9BeautyQSearchEvalQueryDatasetStaticRows" should {
    ...
    "map representative BeautyQ dataset queries into rows accepted by the static runner" in {
      ...
      result match {
        case Right(staticRun) =>
          ...
          assert(staticRun.qualityGateSummary.totalRows == 89)
          assert(staticRun.qualityGateSummary.warningRows == 89)
    ...
    "carry dataset metadata into notes and summary" in {
      ...
      assert(result.summary.fullDatasetQueryCount == 89)
      assert(result.summary.mappedRowCount == 89)
      assert(result.summary.representativeQueryIds == List("q_nails_001", "q_nails_003", "q_noise_005"))
      assert(result.summary.fullAcceptedQueryExpansionImplemented)
      assert(notes.contains("version=1"))
      assert(notes.contains("full_query_count=89"))
      assert(notes.contains("ru=54, en=22, de=6, mixed=7"))
      assert(notes.contains("variantCarousel, providerCarousel, serviceIntentCarousel"))
      assert(notes.contains("Full 89-query static-row expansion implemented"))
    }
    ...
    "validate full dataset presence through bounded resource anchors without full JSON parsing" in {
      ...
      assert(M9BeautyQSearchEvalQueryDatasetStaticRows.StaticQueryIds.size == 89)
      ...
      assert(summary.fullAcceptedQueryExpansionImplemented)
      assert(M9BeautyQSearchEvalQueryDatasetStaticRows.RequiredResourceAnchors.contains("\"queryCount\": 89"))
    }
    ...
    "mark full JSON parsing as deferred while full static-row expansion is implemented" in {
      ...
      assert(warningsAndNotes.contains("Full 89-query static-row expansion implemented: mapped_row_count=89; full_dataset_query_count=89"))
