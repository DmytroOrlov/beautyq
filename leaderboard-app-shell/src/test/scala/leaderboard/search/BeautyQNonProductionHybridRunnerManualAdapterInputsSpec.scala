...
final class BeautyQNonProductionHybridRunnerManualAdapterInputsSpec extends AnyWordSpec {

  "BeautyQNonProductionHybridRunnerManualAdapterInputs" should {

    "adapter inputs, toManualInputs, and buildHandle are side-effect-free" in {
      val inputs = BeautyQNonProductionHybridRunnerManualAdapterInputs(
        lexicalBackend = new FailIfCalledLexicalBackend,
        readinessConfig = testReadinessConfig,
        collectionInfoClient = new FailIfCalledCollectionInfoClient,
        snapshotProvider = new FailIfCalledSnapshotProvider,
        pointUpsertClient = new FailIfCalledUpsertClient,
    ...
    "built handle indexSnapshot delegates through compatibility guard and indexer explicitly" in {
      val expectedVariantId = UUID.fromString("00000000-0000-0000-0000-000000000101")
      val doc = testDocumentWithId(expectedVariantId)

      val inputs = BeautyQNonProductionHybridRunnerManualAdapterInputs(
        lexicalBackend = new FailIfCalledLexicalBackend,
        readinessConfig = testReadinessConfig,
        collectionInfoClient = new ConstCollectionInfoClient(Right(collectionInfoJson())),
        snapshotProvider = new ScriptedSnapshotProvider(List(doc)),
        pointUpsertClient = new ExpectingUpsertClient(
          testReadinessConfig.collectionName,
          expectedVariantId,
        ),
        embeddingClient = new ExpectingEmbeddingClient(doc.allText),
        qdrantSearchClient = new FailIfCalledQdrantSearchClient,
        documentLookup = new FailIfCalledDocumentLookup,
        documentSpec = testDocumentSpec,
        embeddingSpec = testEmbeddingSpec,
    ...
    "built handle run delegates through Qdrant semantic search without indexing" in {
      val expectedVariantId = UUID.fromString("00000000-0000-0000-0000-000000000101")
      val semanticDoc = testDocumentWithId(expectedVariantId)
      ...
      val inputs = BeautyQNonProductionHybridRunnerManualAdapterInputs(
        ...
        snapshotProvider = new FailIfCalledSnapshotProvider,
        pointUpsertClient = new FailIfCalledUpsertClient,
        embeddingClient = new ExpectingEmbeddingClient(input.query),
        qdrantSearchClient = new ExpectingQdrantSearchClient(
    ...
    "missing lookup document propagates through built handle.run" in {
      ...
      val inputs = BeautyQNonProductionHybridRunnerManualAdapterInputs(
        ...
        snapshotProvider = new FailIfCalledSnapshotProvider,
        pointUpsertClient = new FailIfCalledUpsertClient,
        embeddingClient = new ConstEmbeddingClient(Vector(0.5)),
        qdrantSearchClient = new ExpectingQdrantSearchClient(
          testReadinessConfig.vectorSearchSpec.collectionName,
          missingId,
          0.75,
  ...
  private val testEmbeddingSpec: leaderboard.search.dsl.EmbeddingSpec[VariantSearchDocument] =
    leaderboard.search.dsl.EmbeddingSpec(
      ...
      sourceTextFields = List(leaderboard.search.document.BeautyQVariantSearchDocumentContract.Fields.allText),
    )
