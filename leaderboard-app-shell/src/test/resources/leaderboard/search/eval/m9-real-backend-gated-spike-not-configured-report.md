...
|---|---|---|---|---|---|---|---|
| q_spike_exact_001 | exact_product_name_brand | es_only | es | - | failure_count=1 | failed | gate_status=allowed_without_resource_config, backend failure: es offline adapter resource config is not configured; no real backend call attempted |
| q_spike_semantic_001 | semantic_descriptive | es_only | es | - | failure_count=1 | failed | gate_status=allowed_without_resource_config, backend failure: es offline adapter resource config is not configured; no real backend call attempted |
