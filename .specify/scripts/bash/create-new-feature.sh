...
if [ "$DRY_RUN" != true ]; then
    ...
    # A symlinked spec.md leaf is followed by the redirection below (a dangling
    # link also defeats the -f check), writing outside the feature directory.
    # Reject it before any classification, write, or pointer persistence.
    if [ -L "$SPEC_FILE" ]; then
        >&2 echo "Error: Refusing to use symlinked spec.md as write destination: $SPEC_FILE"
        exit 1
    fi
    ...
    if [ ! -f "$SPEC_FILE" ]; then
        ...
        if SPEC_TEMPLATE_CONTENT=$(resolve_template_content "spec-template" "$REPO_ROOT"; status=$?; printf x; exit "$status"); then
            ...
            resolve_status=$?
            # The spec template is required: fail hard instead of materializing
            # an empty placeholder and persisting feature state.
            >&2 echo "Error: Required spec-template could not be resolved (resolver status $resolve_status). No feature directory, spec.md, or feature state was created."
            exit "$resolve_status"
        fi
    fi
